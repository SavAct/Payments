...
public:
    ...
    enum KeyType { K1 = 0, R1 = 1, WebAuthN = 2};
	...
	*/
    ...
    static void checkSigCheckSum(const ecc_signature& ecc_sig_data, const KeyType& sigType, const unsigned char* checkSum){
        ...
        switch (sigType) {
            case KeyType::K1: sig_data[65] = 'K'; sig_data[66] = '1'; break;
            case KeyType::R1: sig_data[65] = 'R'; sig_data[66] = '1'; break;
            default: check(false, "Not supported signature type.");
        }
    ...
    static public_key EccToPubKey(const ecc_public_key ecc_key, const KeyType keyType){
        public_key pubkey; 
        switch (keyType) {
            case K1: pubkey.emplace<0>(ecc_key); break;
            case R1: pubkey.emplace<1>(ecc_key); check(false, "Please, use a public key that beginns with EOS or PUB_K1_."); break; // Because ECC of EOSJS is not able to sign R1 keys
            default: check(false, "Invalid public key type."); break;
        }
        return pubkey;
    }
    ...
    static ecc_public_key PubKeyToEcc(const public_key pub_key, const KeyType keyType){
        ...
        switch(keyType){
            case KeyType::K1: ecc_key = std::get<0>(pub_key); break;
            case KeyType::R1: ecc_key = std::get<1>(pub_key); check(false, "Please, use a public key that beginns with EOS or PUB_K1_."); break; // Because ECC of EOSJS is not able to sign R1 keys
            default: check(false, "This public key format is not considered.");
        }
        return ecc_key;
    }
    ...
    static signature EccToSignature(const ecc_signature ecc_sig, const KeyType sigType){
        signature sig; 
        switch (sigType) {
            case KeyType::K1: sig.emplace<0>(ecc_sig); break;
            case KeyType::R1: sig.emplace<1>(ecc_sig); check(false, "Please, use a signature that beginns with PUB_K1_."); break; // Because ECC of EOSJS is not able to sign R1 keys
            default: check(false, "Invalid signature type."); break;
        }
        return sig;
    }
    ...
    enum ActionType { PAY, RAM, REJ, FIN, OFF, ALL, ACC, INV };
			...
			switch(type){
                case -1:
                    ...
                    if(parameter.size() == 3){
                        switch (parameter[0])
                        {
                            case 'P': p.actionType = ActionType::PAY; break;
                            case 'R': p.actionType = ActionType::RAM; break;
                            case 'F': p.actionType = ActionType::FIN; break;
                            case 'I': p.actionType = ActionType::INV; break;
                            case 'O': p.actionType = ActionType::OFF; break;
                            case 'A': p.actionType = parameter[1] == 'C'? ActionType::ACC : ActionType::ALL; break;
                        default:
                            p.from = parameter;
                            p.hasFrom = true;
                            break;
